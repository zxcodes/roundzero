...
export async function checkAndLaunchBatch(jobId: string): Promise<PoolCheckResult> {
  const db = getDb();
  try {
    const result = await db.begin(async (tx): Promise<PoolCheckResult> => {
      ...
      const oldestQueuedMs =
        pool.length > 0 ? Date.now() - new Date(pool[0].createdAt).getTime() : 0;
    ...
    // Trigger the batch orchestration workflow OUTSIDE the transaction so the
    // DB connection is not held while making an external API call.
    if (result.launched) {
      try {
        await env.BATCH_ORCHESTRATION.create({
          id: result.batchId,
          params: { batchId: result.batchId, jobId: jobId },
        });
      } catch (error) {
        console.error(`Failed to trigger batch orchestration for batch ${result.batchId}`, error);
      }
    }
...
export async function releaseBatchAndNotify(batchId: string): Promise<BatchReleaseSummary> {
  const db = getDb();
  try {
    const summary = await releaseBatch(db, batchId);
