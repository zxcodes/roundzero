...
RETURNING id, interview_id, application_id, summary, strengths, weaknesses, insights, evidence, scores, recommendation, created_at`;
...
export async function createReport(sql: Sql, args: createReportArgs): Promise<createReportRow | null> {
    const rows = await sql.unsafe(createReportQuery, [args.interviewId, args.applicationId, args.summary, args.strengths, args.weaknesses, args.insights, args.evidence, args.scores, args.recommendation]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
