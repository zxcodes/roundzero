...
export async function createPreEvaluation(
  sql: Sql,
  args: createPreEvaluationArgs,
): Promise<createPreEvaluationRow | null> {
  const rows = await sql
    .unsafe(createPreEvaluationQuery, [
      args.applicationId,
      args.score,
      args.missingRequirements,
      args.confidence,
      args.nextStep,
    ])
    .values();
...
export async function getPreEvaluationByApplicationId(
  sql: Sql,
  args: getPreEvaluationByApplicationIdArgs,
): Promise<getPreEvaluationByApplicationIdRow | null> {
  const rows = await sql
    .unsafe(getPreEvaluationByApplicationIdQuery, [args.applicationId])
    .values();
  if (rows.length !== 1) {
    return null;
  }
